...
Security: the bucket is `private` (403 for all unsigned requests), `/f/:id` is behind oauth2-proxy (only authenticated users can obtain a presigned URL), and presigned URLs are short-lived HMAC tokens (5-min expiry). This is the same model as AWS S3 or Cloudflare R2 — the public endpoint is normal. Set `S3_SET_ACL=0` so LobeHub does not attempt to set per-object ACLs (MinIO returns errors when the bucket doesn't use ACL mode).
